...
public class AddTaskCommand implements BaseCommand {
    ...
    private Task createTask(TaskType taskType, String title, String description,String additionalParam){
        switch (taskType){
            case BUG:
                return taskManagementRepository.createBug(title,description,ParsingHelpers.tryParseEnum(additionalParam, Severity.class));
            case STORY:
                return taskManagementRepository.createStory(title,description,ParsingHelpers.tryParseEnum(additionalParam, StorySize.class));
            case FEEDBACK:
                return taskManagementRepository.createFeedback(title,description,Integer.parseInt(additionalParam));
            default:
                throw new IllegalArgumentException(CANNOT_CREATE_THIS_TYPE_OF_TASK);
        }
    }
}
